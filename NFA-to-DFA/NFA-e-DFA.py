import json
import string
from itertools import chain, combinations


def read_nfa(file_path):
    
    with open(file_path, 'r') as f:
        return json.load(f)


def powerset(iterable):
    
    s = list(iterable)
    return list(chain.from_iterable(combinations(s, r) for r in range(len(s) + 1)))


def nfa_to_dfa(nfa):
    
    alphabet = nfa["alphabet"]
    transitions = nfa["transiction"]
    initial_state = nfa["initial_state"]
    end_states = nfa["end_state"]

    
    nfa_transitions = {}
    for t in transitions:
        init, symbol, end = t["initial"], t["symbol"], t["end"]
        
        
        if end == "null":
            end = []
        elif isinstance(end, str):
            end = [end]

        if (init, symbol) not in nfa_transitions:
            nfa_transitions[(init, symbol)] = set()
        nfa_transitions[(init, symbol)].update(end)

    dfa_states = []
    dfa_transitions = []
    dfa_start = frozenset([initial_state])
    dfa_states.append(dfa_start)

    visited = set()
    queue = [dfa_start]

    while queue:
        current = queue.pop(0)
        if current in visited:
            continue
        visited.add(current)

        for symbol in alphabet:
            new_state = set()
            for substate in current:
                new_state.update(nfa_transitions.get((substate, symbol), []))

            new_state_frozen = frozenset(new_state)
            dfa_transitions.append({
                "initial": ",".join(sorted(current)),
                "symbol": symbol,
                "end": [",".join(sorted(new_state)) if new_state else None]
            })

            if new_state_frozen and new_state_frozen not in dfa_states:
                dfa_states.append(new_state_frozen)
                queue.append(new_state_frozen)

   
    dfa_final_states = []
    for state in dfa_states:
        if any(s in end_states for s in state):
            dfa_final_states.append(",".join(sorted(state)))

    dfa = {
        "alphabet": alphabet,
        "states": [",".join(sorted(s)) for s in dfa_states],
        "transiction": dfa_transitions,
        "initial_state": ",".join(sorted(dfa_start)),
        "end_state": dfa_final_states
    }

    return dfa


def complete_dfa(dfa, sink_name="Z"):
    
    states = set(dfa["states"])
    alphabet = dfa["alphabet"]
    transitions = {(t["initial"], t["symbol"]): t["end"][0] for t in dfa["transiction"]}

    added_sink = False

    for state in list(states):
        for symbol in alphabet:
            if (state, symbol) not in transitions or transitions[(state, symbol)] is None:
                
                for t in dfa["transiction"]:
                    if t["initial"] == state and t["symbol"] == symbol:
                        t["end"] = [sink_name]
                        break
                else:
                    dfa["transiction"].append({
                        "initial": state,
                        "symbol": symbol,
                        "end": [sink_name]
                    })
                added_sink = True

    if added_sink:
        states.add(sink_name)
        for symbol in alphabet:
            dfa["transiction"].append({
                "initial": sink_name,
                "symbol": symbol,
                "end": [sink_name]
            })

    dfa["states"] = list(states)
    return dfa


def rename_states(dfa):
    
    mapping = {}
    letters = list(string.ascii_uppercase)
    
    for i, state in enumerate(dfa["states"]):
        mapping[state] = letters[i] if i < len(letters) else f"S{i}"  
    
   
    dfa["states"] = [mapping[s] for s in dfa["states"]]
    dfa["initial_state"] = mapping[dfa["initial_state"]]
    dfa["end_state"] = [mapping[s] for s in dfa["end_state"]]
    
    new_transitions = []
    for t in dfa["transiction"]:
        new_transitions.append({
            "initial": mapping[t["initial"]],
            "symbol": t["symbol"],
            "end": [mapping[e] for e in t["end"]]
        })
    dfa["transiction"] = new_transitions
    
    return dfa


def save_dfa(dfa, file_path):
    
    with open(file_path, 'w') as f:
        json.dump(dfa, f, indent=4)


def print_dfa(dfa):
    
    print("\n=== DFA GERADO ===")
    print("Alfabeto:", dfa["alphabet"])
    print("Estados:", dfa["states"])
    print("Estado inicial:", dfa["initial_state"])
    print("Estados finais:", dfa["end_state"])

    
    trans_dict = {}
    for t in dfa["transiction"]:
        trans_dict[(t["initial"], t["symbol"])] = ",".join(t["end"])

    
    print("\n=== Tabela de Transições ===")
    header = "Estado | " + " | ".join(dfa["alphabet"])
    print(header)
    print("-" * len(header))

    for state in dfa["states"]:
        row = f"{state:6} | "
        row += " | ".join(
            trans_dict.get((state, symbol), "-") for symbol in dfa["alphabet"]
        )
        print(row)


if __name__ == "__main__":
    input_file = "/home/italo/Área de trabalho/italo/NFA-to-DFA/exemplo02.json"
    output_file = "dfa.json"

    nfa = read_nfa(input_file)
    dfa = nfa_to_dfa(nfa)
    dfa = complete_dfa(dfa, sink_name="Z")  
    dfa = rename_states(dfa)  
    save_dfa(dfa, output_file)
    print_dfa(dfa)

    print("\nConversão concluída! DFA salvo em", output_file)
